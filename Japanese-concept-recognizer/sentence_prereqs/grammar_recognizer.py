from consts import pos_tag_translations, pos_tag_encodings
import re
import romkan


ru_verb_exceptions = [x.strip() for x in open("ru-verb-exceptions.txt", "r").readlines()]
...
def pos_tag(sentence, kuro_server):
    ...
    for elem in [[
        ...
        if ru_or_u:
            tagged_words.append([elem[0], [*elem[1], ru_or_u], elem[2], romkan.to_roma(elem[0]), elem[3]])
        else:
...
def recognize_i_adverb(encoded):
    pattern = create_word_pattern(tags=[24]) + create_word_pattern(tags=[5])
...
def recognize_te(encoded):
    ...
    return recognize_pattern(encoded, pattern, "te form positive")
